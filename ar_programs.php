...
$today = date("Y-m-d");    
mysql_select_db($database_conn, $conn);
$query_programs = "SELECT ID, ArTitle, ImageURL, TripCode, Price, DateFrom, DateTo,ArBriefDetails, SpecialOffer, ThumPath FROM tourismprogram WHERE IsActive=1 and DateFrom >=".$today." and tourismprogram.FK_TourismType=".GetSQLValueString($colname_tourism_type, "int")." AND tourismprogram.FK_CityID=".GetSQLValueString($colname_city, "int");
...
<ul id="sd-nav">
 ...
 if($totalRows_programs >0)
 {
	  do{
		  if($programs_counter == 1)
		  {
		  ?>
        ...
        <?php }?>
          <article class="clear boxes-intrnl">
            <figure class="post-image">
            
 <a href="ar_programs_details.php?type=<?php echo  $_REQUEST['type']; ?>&id=<?php echo $row_programs['ID']; ?>"><img src="<?php echo $row_programs['ThumPath']; ?>" alt="القاهرة 4 أيام / 3 ليالي" title="القاهرة 4 أيام / 3 ليالي"></a></figure>
            <header style="text-align:center;">           
              <h2><strong><a href="internal-sidemenu-ar.php"> <?php echo $row_programs['ArTitle']; ?></a></strong></h2>
              <h3>كود الرحلة :<strong> <?php echo $row_programs['TripCode']; ?></strong></h3>
              <h4> <?php echo   $row_programs['ArBriefDetails'];?></h4>
              <h2><strong>السعر  <?php echo $row_programs['Price']; ?> جنيه</strong></h2>
             <h3><strong>
              ...
              &nbsp;
              <?php }?></strong></h3>
              <h6><strong>العرض ساري من  <?php echo $row_programs['DateFrom']; ?> الي  <?php echo $row_programs['DateTo']; ?></strong></h6>
            </header>
            <footer class="read-more"><a href="ar_programs_details.php?type=<?php echo  $_REQUEST['type']; ?>&id=<?php echo $row_programs['ID']; ?>">إقرأ المزيد &raquo;</a></footer>
          </article>
        </li>
        <?php }while($row_programs = mysql_fetch_assoc($programs));
		
		
 } else { ?>
    <div align="center"><h2>لا توجد عروض في الوقت الحالي</h2></div>
...
<!-- InstanceEnd --></html>
<?php
mysql_free_result($tourism_type);
